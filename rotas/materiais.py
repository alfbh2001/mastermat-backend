...
@router.delete("/{id_material}") # Mapeia para DELETE /api/material/{id_material}
def deletar_material(id_material: str):
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cursor = conn.cursor()
        cursor.execute("DELETE FROM mm_material WHERE id_material = %s;", (id_material,))
        conn.commit()
        cursor.close()
        conn.close()
        return {"sucesso": True, "mensagem": "Material deletado com sucesso!"}
    except Exception as e:
        return {"erro": str(e)}
